...
def cars_view(request):
    cars_list = list(cars_collection.find({"status": "Available"}))
    # Récupérer tous les managers et créer un dictionnaire par ID
    managers = {manager['id']: manager for manager in managers_collection.find()}
    
    context = {
        "cars_list": cars_list,
        "managers_dict": managers  # Passer le dictionnaire au contexte
    }
...
def cnx_view(request):
    if request.method == 'POST':
        ...
        if admin and check_password(password, admin['password']):
            request.session['isloggin_admin'] = True
            request.session['admin_id'] = str(admin['_id'])  # Stocker l'id MongoDB en string
            request.session['admin_username'] = admin['username']  # Optionnel, pour affichage
        ...
        if manager and check_password(password, manager.get('password')):
            request.session['isloggin_manager'] = True
...
def dashboard_Admin_view(request):
    ...
    if not request.session.get('isloggin_admin'):
        return redirect('login')

    # Récupérer l'admin connecté via l'id stocké dans la session
    ...
    count_cars = cars_collection.count_documents({})
    count_reserv = reservations_collection.count_documents({})
    count_client = clients_collection.count_documents({})
    count_managers = managers_collection.count_documents({})

    admin_image = admin.get('image', None) if admin else None
...
def add_manager_view(request):
    if not request.session.get('isloggin_admin'):
        return redirect('login')  # Redirige vers login.html si pas connecté
        ...
        if photo:
            ...
            with open(image_path, 'wb') as f:
                for chunk in photo.chunks():
                    f.write(chunk)
            update_data['image_path'] = photo.name
...
def generate_pdf(request, reservation_id, facture_id):
    ...
    # Titre
    elements.append(Paragraph(f'Reservation Invoice #{facture_id}', blue_title_style))
    ...
    # Contact
    contact_info = Paragraph("If you have any questions about this invoice, please contact us at Rentlayt@gmail.com")
